...
//*********************************
// Functions to process random data into probability data
//
// 1.01:
// 		Changed "#include <TransformAxis>" to "#include <TransformAxis1.2>"
//		Changed name of file to ProcessProbability1.01.ipf
// 1.02:	JW 140922
//		probAxisProcess() now automatically removes NaNs. It could make the index wave option less useful if there are NaNs in the input.
//*********************************
...
Function probAxisProcess(w1,processedName, DoPercentOutput, MakeSortIndex)
	...
	if (returnValue == 0)
		if (CheckName(processedName, 1) == 0)
			Duplicate/O w1, $(processedName+"_P")/WAVE=outPW
			Duplicate/O w1, $(processedName+"_X")/WAVE=outXW
			
			Duplicate/O w1, $(processedName+"_I")/WAVE=outIW
			...
			if (nNans)
				Variable finalPoints = numpnts(w1)-nNans
				Redimension/N=(finalPoints) outPW, outXW, outIW
				outPW = (p+.5)/numpnts(outPW)
			endif
