...
// Version 6.38, JP:		A focused Color Wheel graph no longer prevents the graph traces from updating.
//
//			The color wheels colorize the waves in the top graph,
//			with given lightness and saturation starting hue.
//			Lightness, saturation and starting hue vary between 0 and 1.
//
//				NOTE: lightness and saturation are really cheap approximations.
//				For that matter, so is hue, which is simply an rgb circle.
//				Colors appear evenly distributed in "hue", except around green-blue,
//				where they move more quickly, since color perception isn't as good there.
//				I generally call it with lightness=0.9 and saturation=1.
//
// Version 9.02, JP:		Added a way to colorize traces using color table WAVEs (which are different from Igor's built-in color tables).
// Version 9.03, BMB: 	Removed code that deletes the include after the MakeTracesDifferent panel is killed.
//								Deleting the include can break code that might also rely on that package (e.g. PCADemo.pxp) 
...
Function CreateKBColorizePanel()
	...
	NewPanel /W=(412,64,715,647)/N=KBColorizePanel/K=1 as "Make Traces Different"
	...
	String topGraphHost=KBActiveGraphHost()
	if( strlen(topGraphHost) )
		AutoPositionWindow/M=0/R=$topGraphHost KBColorizePanel
	endif

	GroupBox markersGroup,pos={10,9},size={280,78},title="Markers Quick Set"
	PopupMenu allMarkers,pos={23,33},size={152,20},proc=KBColorizeTraces#AllMarkersPopMenuProc,title="Reset All Traces To:"
	PopupMenu allMarkers,mode=20,popvalue="",value= #"\"*MARKERPOP*\""
	PopupMenu markersSeries,pos={34,60},size={183,20},proc=KBColorizeTraces#UniqueMarkersPopMenuProc,title="Unique Markers:"
	...
	PopupMenu allLineStyles,pos={23,122},size={232,20},proc=KBColorizeTraces#AllLineStylesPopMenuProc,title="Reset All Traces To:"
	PopupMenu allLineStyles,mode=1,bodyWidth= 130,popvalue="",value= #"\"*LINESTYLEPOP*\""
	Button uniqueLineStyles,pos={124,150},size={130,20},proc=KBColorizeTraces#UniqueLineStylesButtonProc,title="Sequential Line Styles"

	GroupBox colorsQuickSetGroup,pos={10,185},size={280,147},title="Colors Quick Set"
	PopupMenu ColorPop,pos={23,210},size={152,20},proc=KBColorizeTraces#ColorizePopMenuProc,title="Reset All Traces To:"
	...
	PopupMenu ColorTablePop,pos={52,238},size={223,20},proc=KBColorizeTraces#KBColorTablePopMenuProc,title="Set Traces To:"
	...
	Button colorTableWaves,pos={124,270},size={150,20}
	Button colorTableWaves,title="Color Table Waves ...", proc=ColorTableWavesButtonProc
	Button commonColorsButton,pos={124,301},size={150,20},proc=CommonColorsButtonProc,title="Commonly-Used Colors"
	...
	GroupBox colorWheelGroup,pos={10,337},size={280,220},title="Color Wheel"
	TitleBox hueTitle,pos={26,358},size={66,16},title="Starting Hue",frame=0
	TitleBox saturationTitle,pos={123,358},size={54,16},title="Saturation",frame=0
	TitleBox lightnessTitle,pos={210,358},size={50,16},title="Lightness",frame=0
	// the global variables don't exist until RestoreKBColorizePanelSettings is called, so they're set there, not here
	Slider hueSlider,pos={24,378},size={19,123},proc=KBColorizeTraces#KBJPColorizeSliderProc
	...
	Slider satSlider,pos={120,378},size={19,123},proc=KBColorizeTraces#KBJPColorizeSliderProc
	...
	Slider lightSlider,pos={207,378},size={19,123},proc=KBColorizeTraces#KBJPColorizeSliderProc
	...
	CheckBox linearHue,pos={31,532},size={76,16},proc=KBColorizeTraces#LinearCheckProc,title="Linear Hue "
	...
	DefineGuide UGH0={FT,383},UGH1={FT,494}
...
Function KBColorizeTracesOptLinear(lightness, saturation, startingHue,[useLinearHue])
	...
	numTraces = KBTracesInGraph(graphName)
	if (numTraces <= 0)
		return 0
	endif
	...
	for( traceIndex= 0; traceIndex < numTraces; traceIndex += 1 )
		...
		ModifyGraph/W=$graphName/Z rgb[traceIndex]=(red, green, blue)
	endfor
...
Static Function ColorizePopMenuProc(ctrlName,popNum,popStr) : PopupMenuControl
	...
	ModifyGraph/W=$KBActiveGraph() rgb=(V_Red, V_Green, V_Blue)
End

Function ColorTableWavesButtonProc(ctrlName) : ButtonControl
	String ctrlName
	KBShowColorTableWavePanel()
End
...
static Function AllMarkersPopMenuProc(ctrlName,popNum,popStr) : PopupMenuControl
	...
	StoreKBColorizeSettings()	
	ModifyGraph/W=$KBActiveGraph() marker=(popNum-1)
End
...
//value= #"\"All;Only Filled;Only Outlined;Only Lines;Only Round;Only Square;Only Diamond;Only Triangle;Only Crosses;Random;\""
	...
	for(i=0; i<numTraces; i+=1)
		...
		ModifyGraph/W=$graphName marker[i]=markers[row]
	endfor
...
static Function AllLineStylesPopMenuProc(ctrlName,popNum,popStr) : PopupMenuControl
	...
	Variable i
	for(i=0; i<numTraces; i+=1)
		ModifyGraph/W=$KBActiveGraph() lstyle[i]=popNum-1
	endfor
...
static Function UniqueLineStylesButtonProc(ctrlName) : ButtonControl
	...
	for(i=0; i<numTraces; i+=1)
		...
		ModifyGraph/W=$graphName lstyle[i]=lstyle
	endfor
End
...
static Function/S KBActiveGraphHost()
	String graphName=KBActiveGraph()
...
// *********** Color Table Waves Panel ********************

static Function/S KBColorTableWaveNames([String dataFolder])

	if( ParamIsDefault(dataFolder) )
		dataFolder = "root:Packages:ColorTables"
	endif
	
	String list = KBRecurseColorTablesFolder("",$dataFolder)
	
	Variable items = ItemsInList(list)
	if( items < 1 )
		list ="\M0:(:Click 'Load Color Table Waves' to start;" 
	endif
	return list
End

// returns 1 if w is a color table wave (numeric, 3 or 4 columns, more than 1 row)
// we assume its data range is 0..65535 or can be adjusted if not.
static Function KBIsColorTableWave(WAVE w)

	Variable type = WaveType(w,1)
	if( type != 1 )
		return 0	// not numeric!
	endif
	
	Variable rows=DimSize(w,0),cols = DimSize(w,1)
	if( rows < 2 || cols < 3 )
		return 0
	endif
	// it is unlikely that a matrix with "a lot" of columns is a color table wave
	if( cols > 4 )	// some color table waves might have col[3] = alpha
		return 0
	endif
	return 1
End

static Function/S KBRecurseColorTablesFolder(String listSoFar, DFREF dfr)

	if( !DataFolderRefStatus(dfr) )
		return listSoFar
	endif

	// list all of the waves in the folder
	Variable haveOneWave= 0 // if a folder has at least one color table wave, list disabled folder name in front of it.
	Variable index = 0
	do
		...
		// see if it is a color table wave
		if( KBIsColorTableWave(w) )
			if( haveOneWave == 0 )
				DFREF dfSav = GetDataFolderDFR()
				SetDataFolder dfr
				String dataFolderName = GetDataFolder(0)
				SetDataFolder dfSav
				listSoFar += "\M0:(:" + dataFolderName+";" // append disabled folder name
				haveOneWave=1
			endif
			listSoFar += NameOfWave(w)+";"
		endif
		index += 1
	while(1)
	
	// then recurse into all subfolders
	...
	for(i=0; i<numChildDataFolders; i+=1)
		...
		listSoFar = KBRecurseColorTablesFolder(listSoFar, childDFR)
	endfor
...
static Function TraceColorsFromColorTableWave(WAVE ctabwave, Variable reverse)

	String graphName = KBColorizeTraces#KBActiveGraph()
	if (strlen(graphName) == 0)
		return -1
	endif

	Variable numTraces = KBColorizeTraces#KBTracesInGraph(graphName)
	if (numTraces <= 0)
		return -1
	endif
   ...
   if( denominator < 1 )
	...
	Variable numRows= DimSize(ctabwave,0)
	Variable red, green, blue
	Variable i, index
	for(i=0; i<numTraces; i+=1)
		index = round(i/denominator * (numRows-1))	// spread entire color range over all traces.
		...
		ModifyGraph/W=$graphName rgb[i]=(ctabwave[index][0], ctabwave[index][1], ctabwave[index][2])
	endfor
	return 0
End

static Function/WAVE FindNamedWave(String name [,String dataFolder])

	if( ParamIsDefault(dataFolder) )
		dataFolder = "root:Packages:ColorTables"
	endif
	DFREF dfr = $dataFolder
	WAVE/Z w= FindNamedWaveDFR(name, dfr)
	return w
End

static Function/WAVE FindNamedWaveDFR(String name, DFREF dfr)
	...
	do
		...
		if( CmpStr(name,wn) == 0 && KBIsColorTableWave(w) )
			return w // FOUND IT
		endif
		index += 1
	while(1)
	...
	for(i=0; i<numChildDataFolders; i+=1)
		String childDFName = GetIndexedObjNameDFR(dfr, 4, i)
		DFREF childDFR = dfr:$childDFName
		WAVE/Z w= FindNamedWaveDFR(name, childDFR)
		if( WaveExists(w) )
			return w
		endif
	endfor
...
static Function KBUpdateCTabDemo()

	ControlInfo/W=KBColorTableWave ColorTableWaveNames
	WAVE/Z ctab= FindNamedWave(S_value) // okay if it doesn't exist
	ControlInfo/W=KBColorTableWave reverse
	KBDemoColorTableWave(ctab, V_Value)
End

static Function KBReverseCheckProc(ctrlName,checked) : CheckBoxControl
	String ctrlName
	Variable checked
	
	ControlInfo/W=KBColorTableWave ColorTableWaveNames
	WAVE/Z ctab= FindNamedWave(S_value)
	if( WaveExists(ctab) )
		KBDemoColorTableWave(ctab, checked)
		TraceColorsFromColorTableWave(ctab, checked)
	endif
End

static Function KBColorTableWavesPopMenuProc(ctrlName,popNum,popStr) : PopupMenuControl
	String ctrlName
	Variable popNum
	String popStr
	
	String/G root:Packages:KBColorize:gColorTableWaveName= popStr
	WAVE/Z ctab= FindNamedWave(popStr)
	if( WaveExists(ctab) )
		ControlInfo/W=KBColorTableWave reverse
		Variable reversed= V_Value
		KBDemoColorTableWave(ctab, reversed)
		TraceColorsFromColorTableWave(ctab, reversed)
	endif
End


static Function KBLoadColorTableWaves(ctrlName) : ButtonControl
	String ctrlName
	DisplayHelpTopic "Color Table Waves in the Igor Pro Folder"
End

static Function KBCloseColorTableWavePanel(ctrlName) : ButtonControl
	String ctrlName
	Execute/P/Q/Z "KillWindow KBColorTableWave"
End

Function KBShowColorTableWavePanel()
	String graphName = KBActiveGraph()
	if (strlen(graphName))
		DoWindow/F KBColorTableWave
		if( V_Flag == 0 )
			ShowKBColorTableWave()
		endif
		AutoPositionWindow/R=$graphName KBColorTableWave
		DoUpdate
		KBUpdateCTabDemo()
		SetWindow KBColorTableWave, hook(KBColorTableWave) = KBColorizeTraces#KBColorTableWavePanelHook
	endif
End

static Function KBColorTableWavePanelHook(s)
	STRUCT WMWinHookStruct &s
	...
	strswitch(s.eventName)
		case "activate":
			// Handle activate
			String list = KBColorTableWaveNames()
			Variable items= ItemsInList(list)
			ControlInfo/W=KBColorTableWave ColorTableWaveNames
			if( V_Value == 1 ) // 1 normally is a disabled folder name or an instruction
				if( items >= 2 ) // if we have loaded color table waves
					PopupMenu ColorTableWaveNames,mode=2
				endif
			endif
			KBUpdateCTabDemo()
			break
		case "kill":
			DoWindow KBColorizePanel
			if( V_Flag == 0 )
				Execute/P "DELETEINCLUDE <KBColorizeTraces>"
				Execute/P "COMPILEPROCEDURES "
			endif
...
static Function ShowKBColorTableWave()
	DoWindow/K KBColorTableWave
	NewPanel/K=1/W=(581,56,973,219)/N=KBColorTableWave as "Trace Colors from Color Table Wave"
	...
	if( items >= 2 )
		mode= 2
		String name= StrVarOrDefault("root:Packages:KBColorize:gColorTableWaveName","")
		if( strlen(name) ) // find matching item in popup list
			Variable itemNo = WhichListItem(name, list)
			if( itemNo != -1 )
				mode= 1+itemNo
			endif
		endif
	...
	PopupMenu ColorTableWaveNames,pos={21.00,23.00},size={150.00,20.00},proc=KBColorizeTraces#KBColorTableWavesPopMenuProc
	PopupMenu ColorTableWaveNames,title="Color Table Wave:"
	PopupMenu ColorTableWaveNames,mode=mode,value=#"KBColorizeTraces#KBColorTableWaveNames()"

	Variable checked = NumVarOrDefault("root:Packages:KBColorize:gColorTableWaveReversed",0)
	Variable/G root:Packages:KBColorize:gColorTableWaveReversed = checked

	CheckBox reverse,pos={23.00,69.00},size={57.00,16.00},proc=KBColorizeTraces#KBReverseCheckProc
	CheckBox reverse,title="Reverse",variable=root:Packages:KBColorize:gColorTableWaveReversed
	
	Button close,pos={322.00,132.00},size={50.00,20.00},proc=KBColorizeTraces#KBCloseColorTableWavePanel
	Button close,title="Close"
	Button getColorTableWaves,pos={21.00,132.00},size={140.00,20.00},proc=KBColorizeTraces#KBLoadColorTableWaves
	Button getColorTableWaves,title="Load Color Table Waves"
	KBUpdateCTabDemo() // forcibly builds the demo wave
	Display/W=(117,61,371,95)/HOST=# 
	AppendImage/T :Packages:KBColorize:demoColorTableWave
	ModifyImage demoColorTableWave ctab= {*,*,Grays,0}
	ModifyGraph margin(left)=-1,margin(bottom)=-1,margin(top)=-1,margin(right)=-1,frameStyle=1
	...
	SetActiveSubwindow ##
End
static Function/WAVE KBDemoColorTableWave(WAVE/Z w, Variable reversed)
	String ctabName
	...
	if( !WaveExists(w) )
		// make a dummy image
		Make/O/U/I/N=(10,3)/FREE grey = 60000
		WAVE w= grey
	endif
	Variable rows= DimSize(w,0)
	Make/O/N=(rows,2,3) demoColorTableWave
	if( reversed )
		demoColorTableWave = w[(rows-1)-p][r]
	else
		demoColorTableWave = w[p][r]
	endif
	SetDataFolder dfSav
