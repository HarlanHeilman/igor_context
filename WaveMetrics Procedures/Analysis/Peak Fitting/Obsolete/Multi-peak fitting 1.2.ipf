...
Function ResetAxisRange()
	...
	if( cmpstr(gXDataName,"_calculated_") == 0 )
		vmin= pnt2x($gYDataName,gRangeBegin)
		vmax= pnt2x($gYDataName,gRangeEnd)
	else
		Wave w= $gXDataName
		vmin=w[gRangeBegin]
		vmax=w[gRangeEnd]
...
Function ButtonProcSet(ctrlName) : ButtonControl
	...
	Variable/G :WMPeakFits:Current:gRangeBegin,:WMPeakFits:Current:gRangeEnd
	NVAR gRangeBegin= :WMPeakFits:Current:gRangeBegin
	NVAR gRangeEnd= :WMPeakFits:Current:gRangeEnd
...
Window FitSetupPanel() : Panel
	PauseUpdate; Silent 1		// building window...
	String dfSav= GetDataFolder(1)
	
	SetDataFolder root:Packages:WMmpFitting

	NewPanel /W=(302,39,529,445)
	ModifyPanel cbRGB=(32769,65535,32768)
