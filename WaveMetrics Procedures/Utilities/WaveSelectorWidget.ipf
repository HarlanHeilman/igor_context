...
//
//	Version 1.18
// 		JP 141204: WS_SortWaveList uses free waves instead of global waves.
//
//	Version 1.19, kindly contributed by Kevin R. Boyce
// 		KRB 170630: The nameFilterProc is called even when using only WMWS_DataFolders
//
// Version 1.20
//			JW 180214: Allowing data folder filtering always was too broad. Now, by default, data folder names are passed to
//			the filter proc only if WaveSelectorListInfo.contents == WMWS_DataFolders. You can ask to have data folders passed
//			to your filter proc using WS_SetAllowFilteringDataFolders().
//			Also in this version, added WS_SetNameFilterProc().
//
// Version 1.21
//			JP 181108: added WMWS_Killed and WMWS_PopupPanelKilled definitions
//
// Version 1.22
//			JW 200807: added WS_SetListOptionsStr() and WS_GetListOptionsStr()
//			Fixed a bug: WS_SetFilterString() now returns the old filter string as documented
//
//	Version 1.23
//			JW 201012: Fixed a bug: if you clicked in the blank area below the list contents, the selection is removed. But
//			the notification proc was not called.
//			Added some clarification to the documentation for the notification proc.
//	Version 1.24
//			JW 220706:
//			The "content" input to your filter function now properly identifies data folders using the WMWS_DataFolders
//			constant when the list contents are set to WMWS_Waves and you called WS_SetAllowFilteringDataFolders() to allow filtering
//			data folders.
//			Added an optional parameter "includeDFs" to WS_SelectedObjectsList() function so that you can include data folders
//			in the selected objects list even when the list is set to something other than WMWS_DataFolders.
// Version 1.25
//			JW 230214: Added WMWS_NoDFs constant to allow suppression of selection of data folders. See documentation below
//			of the MakeListIntoWaveSelector() function.
// **********************************
...
//								WMWS_NoDFs			don't select data folders. This constant can be AND'ed into the content input
//														to suppress the inclusion of data folders in the selection or notification of
//														the selection of a data folder. That is, use "content = WMWS_Waves | WMWS_NoDFs"
//														to suppress notifications and selection of data folders. Note that it is impossible
//														to completely eliminate selection of data folders, but they will never be included
//														in the list of selected objects, and it will not cause a call to the
...
//
//	WS_SelectedObjectsList(windowname, listcontrolname [, includeDFs])
//
//	Returns a semicolon-separated list of selected items.
//		The optional parameter includeDFs controls whether to include selected data folders
//		in the returned list. If you you created the widget with content set to WMWS_DataFolders,
//		or if includeDFs is present and set to non-zero, data folders are included in the list.
//		If WMWS_noDFs was set in the content input when the widget was created, and content is NOT WMWS_DataFolders,
//		then data folders are NOT included, even if includeDFs is non-zero.
...
//
//		NOTE: the named item is the item that appears on the row that was clicked. It may have been selected or de-selected.
//				If you click in the blank area below the last item in the list your notification proc will be called
...
//
// WS_SetListOptionsStr(windowname, listcontrolname, newoptionsstr)
//		Sets the options string used with WaveList() when getting the list of waves to display.
//		The previous options string is the return value.
//
// WS_GetListOptionsStr(windowname, listcontrolname)
//		Returns the current list options string.
//
// WS_SetNameFilterProc(windowname, listcontrolname, filterProcNameStr)
//		Sets the nameFilterProc to the funtion named by filterProcNameStr. Setting filterProcNameStr to "" disables filtering.
//		Returns the old filter proc name.
//
// WS_SetAllowFilteringDataFolders(windowname, listcontrolname, allowFilterDFs)
//		By default, the NameFilterProc only filters objects that are not data folders, unless the widget contents is
//		WMWS_DataFolders. If allowFilterDFs is set to non-zero, then data folders are passed to your filter proc as well.
// **********************************
...
Constant WMWS_NoDFs = 1024				// can be and'ed with the other constants. If set, data folders are not included in lists of selection, or notifications. We make an attempt to prevent selection of DF's.
...
// Version 1.21
Constant WMWS_Killed = 6
Constant WMWS_PopupPanelKilled = 7 // for PopupWaveSelector.ipf
...
static Constant WaveSelectorVersion = 1.20		// added allowFilterDataFolders
...
static Structure WaveSelectorListInfo
	...
	int32	RootFolderRow
	int16 allowFilterDataFolders			// When non-zero, passes data folder names through the nameFilterProc as well as other object names.
													// If contents is WMWS_DataFolders, then data folders are always passed through the filter proc
	int16 dontSelectDataFolders				// Set if MakeListIntoWaveSelector() input variable "content" includes WMWS_noDFs
EndStructure
...
Function MakeListIntoWaveSelector(windowname, listcontrolname, [content, selectionMode, listoptions, matchStr, nameFilterProc])
	...
	if (content & WMWS_noDFs)
		dontSelectDFs = 1
		content = content & ~WMWS_noDFs
		if (content == WMWS_DataFolders)
			dontSelectDFs = 0						// programmer error!
		endif
	endif
	...
	ListInfo.dontSelectDataFolders = dontSelectDFs
	...
	if (ListInfo.dontSelectDataFolders)
		SelWave[0][1] = 0x80		// disables the data folder cell
	endif
...
Function/S WS_SelectedObjectsList(String windowname, String listcontrolname [, Variable includeDFs])
	STRUCT WaveSelectorListInfo ListInfo
	String userdata = GetUserData(windowname, listcontrolname, "WaveSelectorInfo")
	StructGet/S ListInfo, userdata
	if (ListInfo.version == 0)
	...
	// JW 230215 There are ways to select a data folder, even if WMWS_noDFs is set. So, if it is set, we
	// maintain the fiction that DFs aren't selectable by not reporting any selected DFs
	if (ParamIsDefault(includeDFs) || ListInfo.dontSelectDataFolders)
		includeDFs = 0
	endif
	...
	String theList = ""
	for (i = 0; i < nrows; i += 1)
		if (!includeDFs && (ListInfo.contents != WMWS_DataFolders) && (SelWave[i][0] & 0x40) )
	...
	for (i = 0; i < nrows; i += 1)
		...
		if (CmpStr(ListWave[i][1][1], FolderPathToOpen) == 0)
			if ( (SelWave[i][0] & 0x10)  == 0)
				...
				if (ListInfo.dontSelectDataFolders)
					SelWave[i][1] = 0x80		// disables the data folder cell
				endif
			endif
			break;
		endif
...
static Function OpenAFolder(ListInfo, DataFolderRow)
	...
	if (ListInfo.contents != WMWS_DataFolders)
		...
		if (strlen(ListInfo.nameFilterProcStr) > 0)
			...
			for (i = 0; i < oldNObjects; i += 1)
				...
				String fullname = parentDFwColon+PossiblyQuoteName(aName)
				if (namefilterproc(fullname, ListInfo.contents))
					objects += aName+";"
					nObjects += 1
				endif
			endfor
	...
	do
		...
		// +++ KRB 2017-06-29 Apply filter proc if one was provided
		// JW 230215 Even if WMWS_noDFs is set, it is still useful to filter data folders. If the client code sets
		// allowFilterDataFolders, then we should allow filtering data folders. We just won't select them.
		if ( strlen(ListInfo.nameFilterProcStr) > 0 && (ListInfo.contents == WMWS_DataFolders || ListInfo.allowFilterDataFolders) )
			FUNCREF namefiltertemplate namefilterproc = $ListInfo.nameFilterProcStr
			if ( namefilterproc( parentDFwColon+PossiblyQuoteName( objName ), WMWS_DataFolders ) )
				ListWave[listrow][0][1] = parentDF
				ListWave[listrow][1][1] = parentDFwColon+PossiblyQuoteName(objname)
				ListWave[listrow][1][0] = indentString+objname
				SelWave[listrow][0] = 0x40
				// JW 230215 disable the data folder name cell to prevent selection (in most cases)
				if (ListInfo.dontSelectDataFolders)
					SelWave[listrow][1] = 0x80
				endif
				listrow += 1
			else
				nRowsTooMany += 1
			...
			// if we're not filtering data folders, include this one
			ListWave[listrow][0][1] = parentDF
			ListWave[listrow][1][1] = parentDFwColon+PossiblyQuoteName(objname)
			ListWave[listrow][1][0] = indentString+objname
			SelWave[listrow][0] = 0x40
	...
	// +++ KRB 2017-06-30 Remove extra rows if needed
	if (nRowsTooMany)
...
Static Function WMWS_ObjectIsDF(String object)
	DFREF df = $object
...
Static Function CallNotificationProc(object, event, windowName, listName, procName, isExtended, dontNotifyDFs)
	String object
	Variable event
	String windowName, listName
	String procName
	Variable isExtended
	Variable dontNotifyDFs
	
	if (event == WMWS_DoubleClick || event == WMWS_SelectionChanged || event == WMWS_SelectionChangedShift)
		if (dontNotifyDFs && WMWS_ObjectIsDF(object))
			return 0
		endif
	endif
	...
	switch(LB_Struct.eventCode)
				...
				if (SelWave[LB_Struct.row][0] & 0x40)
					if (SelWave[LB_Struct.row][0] & 0x10)	// if it's checked, it needs to be opened
						...
						if (ListInfo.NotificationProc[0] != 0)
							CallNotificationProc(ListWave[LB_Struct.row][1][1], WMWS_FolderOpened, LB_Struct.win, LB_Struct.ctrlName, ListInfo.NotificationProc, ListInfo.isExtendedProc, ListInfo.dontSelectDataFolders)
						endif
					else
						CloseAFolder(ListInfo, LB_Struct.row)
						if (ListInfo.NotificationProc[0] != 0)
							CallNotificationProc(ListWave[LB_Struct.row][1][1], WMWS_FolderClosed, LB_Struct.win, LB_Struct.ctrlName, ListInfo.NotificationProc, ListInfo.isExtendedProc, ListInfo.dontSelectDataFolders)
						endif
					endif
					DoUpdate
				endif
		...
		case 3:			// double-click
			...
			if (LB_Struct.col == 1)
				...
				elseif ( (SelWave[LB_Struct.row][0] & 0x50) == 0x40 )
					...
					if (ListInfo.NotificationProc[0] != 0)
						CallNotificationProc(ListWave[LB_Struct.row][1][1], LB_Struct.eventCode, LB_Struct.win, LB_Struct.ctrlName, ListInfo.NotificationProc, ListInfo.isExtendedProc, ListInfo.dontSelectDataFolders)
					endif
				endif
			endif
			break;
		...
		case 5:			// shift-selection
			...
			if (LB_Struct.col == 1)
				if (ListInfo.NotificationProc[0] != 0)
					String selectedName = ""
					if (LB_Struct.row < DimSize(SelWave, 0))
						selectedName = ListWave[LB_Struct.row][1][1]
					endif
					CallNotificationProc(selectedName, LB_Struct.eventCode, LB_Struct.win, LB_Struct.ctrlName, ListInfo.NotificationProc, ListInfo.isExtendedProc, ListInfo.dontSelectDataFolders)
				endif
			endif
			break;
	endswitch
...
static Function/S WS_SortWaveList(list, sortKind)
	...
	Make/O/N=(n)/FREE index=p	// JP141204
	Make/O/D/N=(n)/FREE sortValue
...
Function/S WS_SetListOptionsStr(String windowname, String listcontrolname, String newoptionsstr)
	STRUCT WaveSelectorListInfo ListInfo
	String userdata = GetUserData(windowname, listcontrolname, "WaveSelectorInfo")
	StructGet/S ListInfo, userdata
	
	String oldOptionsStr = 	WS_GetListOptionsStr(windowname, listcontrolname)
	
	if (CmpStr(newoptionsstr, oldOptionsStr) != 0)
		Variable optionsLen = strlen(newoptionsstr)
		if (optionsLen > 100)
			ListInfo.wavelistOptions1 = newoptionsstr[0,99]
			ListInfo.wavelistOptions2 = newoptionsstr[100,optionsLen-1]
		else
			ListInfo.wavelistOptions1 = newoptionsstr
			ListInfo.wavelistOptions2 = ""
		endif
		StructPut/S ListInfo, userdata
		ListBox $listcontrolname, win=$windowname, userData(WaveSelectorInfo)=userdata
	...
	return oldOptionsStr
end

Function/S WS_GetListOptionsStr(String windowname, String listcontrolname)
	STRUCT WaveSelectorListInfo ListInfo
	String userdata = GetUserData(windowname, listcontrolname, "WaveSelectorInfo")
	StructGet/S ListInfo, userdata

	return ListInfo.wavelistOptions1 + ListInfo.wavelistOptions2
end

Function/S WS_SetNameFilterProc(windowname, listcontrolname, filterProcNameStr)
	String windowname, listcontrolname, filterProcNameStr

	STRUCT WaveSelectorListInfo ListInfo
	String userdata = GetUserData(windowname, listcontrolname, "WaveSelectorInfo")
	StructGet/S ListInfo, userdata
	
	String oldFilterProc = 	ListInfo.nameFilterProcStr
	
	if (CmpStr(filterProcNameStr, oldFilterProc) != 0)
		ListInfo.nameFilterProcStr = filterProcNameStr
		StructPut/S ListInfo, userdata
		ListBox $listcontrolname, win=$windowname, userData(WaveSelectorInfo)=userdata
		WS_UpdateWaveSelectorWidget(windowname, listcontrolname)
	endif
	
	return oldFilterProc
end

Function WS_SetAllowFilteringDataFolders(windowname, listcontrolname, allowFilter)
	String windowname, listcontrolname
	Variable allowFilter

	STRUCT WaveSelectorListInfo ListInfo
	String userdata = GetUserData(windowname, listcontrolname, "WaveSelectorInfo")
	StructGet/S ListInfo, userdata
	
	Variable oldAllow = 	ListInfo.allowFilterDataFolders
	
	if (oldAllow != allowFilter)
		ListInfo.allowFilterDataFolders = allowFilter
		StructPut/S ListInfo, userdata
		ListBox $listcontrolname, win=$windowname, userData(WaveSelectorInfo)=userdata
		WS_UpdateWaveSelectorWidget(windowname, listcontrolname)
	endif
