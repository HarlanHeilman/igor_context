...
//	Version 1.12 JP 
//								fpopupWSPanel() title the Sort PopupMenu PopupWS_OptionsMenu.
//								After the sortKind and sortReverse possibly changed in the popup panel
//								are copied back to the host control button or setvariable's PopInfo structure,
//								a new WMWS_PopupPanelKilled message is sent to the host button or setvariable.
// Version 1.13 JW 190606
//								Fixed a bug: the popup window popped up in the wrong place if the button was in a subwindow.
// Version 1.14 JW 190620
//								Added ControlUpdate at start of PopupWS_SVcalcDimensions() because if you write a function that
//								creates the SetVariable control and then immediately calls MakeSetVarIntoWSPopupButton(), the size
//								and position info for the SetVariable have not necessarily been updated yet. On Windows, that resulted
//								in the frame around the controls and the little arrow button at the right end being the wrong size.
// Version 1.15 JW 200420
//								Changed local constant MAX_OBJ_NAME to 101 from 32
// Version 1.16 JW 200714
//								Fixed a bug: the popup window popped up in the wrong place if the button was in a
//								deeply nested subwindow, or in an exterior panel.
// Version 1.17 ST 201027
//								Fixed a bug: because of pixel conversion issues the popup window still popped up in the wrong place
//								on Windows if the button was in a subwindow.
//								Fixed a bug: the popup window popped up in the wrong place if it would have been created partly outside
//								the Igor application window (due to host window placement near the edge). Now the popup window is created
//								at a fixed position inside the application window and moved to its final position in a second step.
//								Added support for panel expansion. The popup window is scaled in the same way as the host panel.
// Version 1.17 JW 201027		Work-around for a floating panel on Macintosh disappearing as soon as it is popped up.
// Version 1.17 ST 201103		Added work-around to properly align a floating panel in Igor 8.
// Version 1.18 JP 210426		MakeSetVarIntoWSPopupButton uses svar gs instead of $globalString. Shipped with Igor 9.
// Version 1.19 JW 230215		Added documentation for new WaveSelectorWidget option "WMWS_NoDFs" that can be used with PopupWaveSelector as well.
// **********************************
...
// PRIVATE STUFF
static Constant MAX_OBJ_NAME = 101					// used for button names and notification proc name
...
static Function PopupWS_SVcalcDimensions(hostWindow, hostSetVar, lineFrame, svd)
	...
	ControlUpdate/W=$hostWindow $hostSetVar
...
Function MakeSetVarIntoWSPopupButton(hostWindow, hostSetVar, notifyProc, globalString [, initialSelection, popupWidth, popupHeight, options, content])
	...
	SetVariable $hostSetVar,win=$hostWindow,noedit=1,value=gs,proc=PopupWSSetVarProc
...
static Function fpopupWSPanel(leftInPoints, topInPoints, hostWindow, hostButton [, row, column, selectableStringList])
	...
	Variable magnification = PanelResolution(hostWindow)/PanelResolution("")		// ST: a way to detect a panels expansion factor => expand WS panel in the same way as the host panel
	...
	// ST: the panel is first created at a fixed position of 100:100 which is mostly guaranteed to be fully inside the Igor window
	// ST: otherwise the panel might be automatically shifted around to fit inside the main window upon creation, which screws up the panel placement later on (GetWindow wsizeOuter)
	Variable alignFloat = 0
#if (IgorVersion() >= 9.00)															// ST: in Igor 9 the panel is expanded
	NewPanel/EXP=(magnification) /FLT=(popInfo.doFloat)/K=1 /W=(100/factor,100/factor,100/factor+popInfo.width,100/factor+popInfo.height) as ""
#else
	NewPanel /FLT=(popInfo.doFloat)/K=1 /W=(100/factor,100/factor,100/factor+popInfo.width,100/factor+popInfo.height) as ""
	alignFloat = popInfo.doFloat ? 4.25 : 0											// ST: will align a floating window correctly in Igor 8 (GetWindow wsizeOuter reports V_left and V_right shifted by 4.25 for some reason here)
#endif
	...
	Variable left	= (leftInPoints+100)/magnification - V_left - alignFloat		// ST: properly account for the host window's magnification factor and fixed placement at creation (100/100)
	Variable top	= (topInPoints+100)/magnification  - V_top
	Variable right	= (leftInPoints+100)/magnification - V_left + popInfo.width*factor - alignFloat
	Variable bottom	= (topInPoints+100)/magnification  - V_top  + popInfo.height*factor
	MoveWindow/W=popupWSPanel left, top, right, bottom
	...
	PopupWSPutStruct(popInfo)
	PopupMenu PopupWS_OptionsMenu,pos={1, popInfo.height-kPopMenuHeight-2},size={71,kPopMenuHeight}, title="Sort By"
...
Function GlobalCtrlCoordsInPixels(hostWindow, ctrlName, left, top, right, bottom, floats)
	...
	Variable magnification = PanelResolution(hostWindow)/PanelResolution("")		// ST: a way to detect a panels expansion factor => control positions need to be adjusted for magnification	
	if (floats)
		ControlInfo/W=$hostWindow/G $ctrlName
		top = V_top
		left = V_left
		right = left + V_width * magnification
		bottom = top + V_height * magnification
	else
		ControlInfo/W=$hostWindow $ctrlName
		top = V_top * magnification
		left = V_left * magnification
		right = left + V_width * magnification
		bottom = top + V_height * magnification
		Variable pathItems = ItemsInList(hostWindow, "#")
		Variable i
		
		// +++++ The following code circumvents inconsistent behavior of GetWindow wsize +++++
		// ST: the code categorizes three window types: host panel -> OS pixel conversion factor; in-between sub-panels -> ignore (0); lowest sub-panel -> use but no conversion (1)
		String CurrentLevel = ""
		Variable isSubPanel = 0
		Make/Free/N=(pathItems) factorWave = 1							// this wave holds a scaling factor for each sub-window level
		for (i = 0; i < pathItems; i++)
			CurrentLevel += SelectString(strlen(CurrentLevel),"","#") + StringFromList(i,hostwindow,"#")
			if (WinType(CurrentLevel) == 7)
				GetWindow $CurrentLevel, exterior						// if an exterior window is detected then this will be the host graph
				if (V_value)
					isSubPanel = 0
				endif
				
				if (isSubPanel)											// all levels after the host panel has been found
					factorWave[i] = 0									// mark sub-panels as such (all besides the last one will get ignored later on)
				else
					factorWave[i] = screenResolution/PanelResolution("")// for host panels apply OS scaling factor WITHOUT panel expansion factor since the top and left coordinates are used (which do not move with expansion)
					isSubPanel = 1										// sub-panels from here on
				endif
			endif
		endfor
		
		Reverse factorWave
		FindValue/V=0 factorWave										// find deepest sub-panel (last item)
		if (V_value > -1)
			factorWave[V_value] = 1										// this sub-panel will have the correct coordinates within the host, so use for addition later on
		endif
		// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		
		for (i = 0; i < pathItems; i++)
			GetWindow $hostWindow, wsize
			left += V_left*factorWave[i]								// add with the factors determined above
			right += V_left*factorWave[i]
			top += V_top*factorWave[i]
			bottom += V_top*factorWave[i]
			
			GetWindow $hostWindow, exterior
			if (V_value)
				break													// An exterior panel is not the root window, but it is the end of the road for this computation
			endif
			hostWindow = ParseFilePath(1, hostWindow, "#", 1, 0)		// mode 1:removes leaf end; whichend = 1: count from the end; whichElement=0, the last element
			hostWindow = RemoveEnding(hostWindow, "#")
		endfor
	endif
...
Function popupWSPanelHook(ws)
	...
	switch(ws.eventCode)
		case 1:							// deactivate
			// JW 201027 This kludgey bit makes it act like it does on Windows, which isn't ideal, but at least you can make a selection.
			// I'm not sure what's changed, but we appear to get a deactivate event right after the panel pops up, and that makes it
			// disappear right away. Apparently on Windows we don't get a deactivate event at all, so it is necessary to click the button
			// itself, or to click the Close button in the popped-up panel, or to make a selection.
#ifdef MACINTOSH
#if IgorVersion() >= 9
			infoString = GetUserData(ws.winname, "", "popupWSInfo")
			StructGet/S popInfo, infoString
			if (popInfo.version == 0)
				return 0
			endif
		...
		case 2:							// kill
			...
			magnification = PanelResolution(ws.winname)/PanelResolution("")					// ST: expand WS panel in the same way as the host panel
			popInfo.width  = (ws.winRect.right - ws.winRect.left)/magnification
			popInfo.height = (ws.winRect.bottom - ws.winRect.top)/magnification
			...
			PopupWSPutStruct(popInfo)
			// Version 1.12: Tell host button the panel is closing
			FUNCREF popupWS_ButtonNotifyTemplate buttonfunc = $popInfo.NotificationProc
			String ctrlName = popInfo.hostButton
			if (strlen(popInfo.hostSetVar) > 0)
				ctrlName = popInfo.hostSetVar
			endif
			buttonfunc(WMWS_PopupPanelKilled, StringFromList(0, WS_SelectedObjectsList("popupWSPanel", "list0")), popInfo.hostWindow, ctrlName)
			break;
		case 11:
			switch(ws.keycode)
				case 27:				// escape
			...
			break;
		case 6:							// resize
			magnification = PanelResolution(ws.winname)/PanelResolution("")					// ST: make sure the controls stay within the expanded panel
			Variable height = (ws.winRect.bottom - ws.winRect.top)/magnification
			Variable width	= (ws.winRect.right - ws.winRect.left)/magnification
			PopupMenu PopupWS_OptionsMenu,win=$ws.winName,pos={1, height-kPopMenuHeight-2}
			ListBox list0,win=$ws.winName,size={width, height-kPopMenuMargin}
			break;
	endswitch
